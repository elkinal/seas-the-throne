 ...
 *
 * <p>The class breaks up the image into regions, according the number of rows and columns in the
 * image. It then indexes each region by frame, starting from the top-left and processing one row at
 * a time.
 *
 * <p>This is a subclass of TextureRegion, and so it keeps a rectangle region that tracks the active
...
public class FilmStrip extends TextureRegion {
   ...
   * Creates a new filmstrip from the given texture.
   *
   * <p>The parameter size is to indicate that there are unused frames in the filmstrip. The value
   ...
   *
   * <p>If the frame index is invalid, an error is raised.
   *
  ...
  public void setFrame(int frame) {
    if (frame < 0 || frame >= size) {
      Gdx.app.error(
          "FilmStrip",
          "Invalid animation frame; got " + frame + " when size was " + size,
