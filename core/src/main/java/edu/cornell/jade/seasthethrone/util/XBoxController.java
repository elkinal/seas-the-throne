 ...
 *
 * <p>This is a wrapper class, which wraps around a Controller object to provide concrete mappings
 * for the buttons, joysticks and triggers. It is simpler than having to remember the exact device
 ...
 *
 * <p>Each controller must have its own instance. The constructor automatically determines what OS
...
public class XBoxController implements Controller, ControllerListener {
   ...
   *
   * <p>Low-level device polling can only ask about individual d-pad buttons. However, the primary
   ...
   *
   * <p>An XBox controller must have an OS supported mapping for its buttons.
   *
   ...
   *
   * <p>Due to joystick sensitivity, we allow some minor dead zone settings. Any joystick value
   * within this distance of 0 will register as 0.
   *
   ...
   *
   * <p>This method returns false if the controller is disconnected.
   *
   ...
   *
   * <p>This is method only returns true if the up button is pressed by itself. If the up button is
   ...
   *
   * <p>The result will be one of the eight cardinal directions, or center if the DPad is not
   * actively pressed.
   *
   * <p>This method returns NEUTRAL if the controller is disconnected.
   *
   ...
   *
   * <p>This is a value between -1 and 1, where -1 is to the left.
   *
   * <p>This method returns 0 if the controller is disconnected.
   *
   ...
   *
   * <p>This is a value between -1 and 1, where -1 is towards the bottom.
   *
   * <p>This method returns 0 if the controller is disconnected.
   *
   ...
   *
   * <p>This is a value between 0 and 1, where 0 is no pressure.
   *
   * <p>This method returns 0 if the controller is disconnected.
   *
   ...
   *
   * <p>This method returns null if the controller is disconnected.
   *
   ...
   *
   * <p>This ID is used to recognize this controller if more than one of the same controller models
   * are connected. Use this to map a controller to a player, but do not use it to save a button
   * mapping.
   *
   * <p>This method returns null if the controller is disconnected.
   *
   ...
   *
   * <p>Axis indices start at 0, so the maximum axis index is one under this value. On XBoxes, the
   * high end axes are the triggers.
   *
   * <p>This method returns 0 if the controller is not connected.
   *
   ...
   *
   * <p>This method returns false if the controller is not connected.
   *
   ...
   *
   * <p>Note that this is no guarantee that the connected controller itself can vibrate. Simply that
   ...
   *
   * <p>This method does nothing if the controller is not connected.
   *
   ...
   *
   * <p>This not be supported by some implementations. This method does nothing if the controller is
   * not connected.
   */
  @Override
   ...
   *
   * <p>This returns PLAYER_IDX_UNSET if none is set, or if the controller is disconnected.
   *
   ...
   *
   * <p>Please note that this does not always set indication lights of controllers. It is just an
   * internal representation on some platforms. Set this to PLAYER_IDX_UNSET to unset the index.
   *
   * <p>This method does nothing if the controller is disconnected.
   *
   ...
   *
   * <p>The connected controller might not support all features. This method returns null if the
   ...
   *
   * <p>This value indicates the battery state of the connected controller, or {@link
   * ControllerPowerLevel#POWER_UNKNOWN} if information is not present (including when the
   ...
   *
   * <p>The listener will receive calls in case the state of the controller changes. The listener
   ...
   *
   * <p>This method does nothing if the controller is disconnected or the given listener is not
   * registered.
   *
   ...
   *
   * <p>The buttonCode is controller specific. The <code>com.badlogic.gdx.controllers.mapping</code>
   ...
   *
   * <p>The axisCode is controller specific. The axis value is in the range [-1, 1]. The <code>
   * com.badlogic.gdx.controllers.mapping</code> package hosts axes constants for known controllers.
   *
   * @param controller The controller interface
