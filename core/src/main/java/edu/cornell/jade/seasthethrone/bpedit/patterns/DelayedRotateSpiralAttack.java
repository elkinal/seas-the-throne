...
public class DelayedRotateSpiralAttack extends AttackPattern {
   ...
   */
  public DelayedRotateSpiralAttack(BossModel model, int delay, int shots, int delayRotate, float theta,
                               boolean unbreakable, BulletModel.Builder builder, PhysicsEngine physicsEngine) {
    this.delay = delay;
    this.angle = MathUtils.PI * 2f / shots;

    this.stepsTaken = 0;
    this.spawner = SpawnerFactory.constructRepeatingDelayedRotateStream(delay, delayRotate, theta, builder, physicsEngine);
